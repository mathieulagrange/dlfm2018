...
The only example of node in the MedleyDB taxonomy \cite{bittner2014ismir} reaching this level is \emph{tack piano}.
...
Unlike musical instruments, which are approximately amenable to a hierarchical taxonomy of resonating objects, playing techniques result from a complex synchronization between multiple gestures, which may involve both hands and arms, diaphragm, vocal tract, and sometime the whole body.
As a result, there is no immediate 
...
% Slow sedimentation of the name of playing techniques. Need to interact with experts of each instrument.
In what follows, we adopt a middle ground position
...
\caption{Partial taxonomies of violin playing techniques.}
