...
For each sound, some features are computed over a given time frame, ranging from 25 ms. to 1 second. The set of frames is averaged to give one feature vector per sound recording.

\ml{shall we discuss the train test split ?}
...
Enlarging the frame size do not influence the peformance of the MFCC baseline. On contrary, as the scattering 

\subsection{Evaluation of playing technique recognition}

When considering the playing technique partition as a reference gives a more complicated task as the  MFCC baseline achieves a p@5 of $45\%$.
